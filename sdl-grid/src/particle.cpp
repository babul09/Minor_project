...
ParticleSystem::ParticleSystem(int maxParticles_)
    : maxParticles(maxParticles_), 
      ...
      noiseSpeed(1.0f), 
      windPhase(0.0f),
      emitterSpeedMin(50.0f), 
      emitterSpeedMax(250.0f), 
      emitterSpread(TWO_PI), 
      emitterDirection(0.0f), 
      emitterRadius(6.0f),
      particleSize(2) {
...
void ParticleSystem::spawnOne(float emit_x, float emit_y) {
    ...
    const float speed = emitterSpeedMin + randf() * (emitterSpeedMax - emitterSpeedMin);
...
void ParticleSystem::render(SDL_Renderer* renderer, int psize, float scale, 
    ...
    for (int i = 0; i < maxParticles; ++i) {
        if (particles[i].life > 0.0f) {
            ...
            const int drawSize = std::max(1, static_cast<int>(std::floor(baseSize * scale + 0.5f)));
            r.w = drawSize;
            r.h = drawSize;
    ...
    for (const auto& c : circles) {
        const int cx = static_cast<int>(std::floor((c.x - centerX) * scale + centerX + 0.5f));
        const int cy = static_cast<int>(std::floor((c.y - centerY) * scale + centerY + 0.5f));
        const int cr = std::max(1, static_cast<int>(std::floor(c.r * scale + 0.5f)));
...
void ParticleSystem::accumulateGrid(const GridRect& gridRect, int rows, int cols, std::vector<int>& outCounts) const {
    ...
    for (const auto& p : particles) {
        ...
        const float px = p.x;
        const float py = p.y;
        if (px < gridRect.x || py < gridRect.y ||
            px >= gridRect.x + gridRect.w || py >= gridRect.y + gridRect.h) {
            continue;
        }
        const int col = static_cast<int>((px - gridRect.x) / static_cast<float>(gridRect.cellSize));
        const int row = static_cast<int>((py - gridRect.y) / static_cast<float>(gridRect.cellSize));
