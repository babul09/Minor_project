...
Boundary::Boundary(int rows_, int cols_) : rows(0), cols(0) {
  resize(rows_, cols_);
}
...
void Boundary::resize(int rows_, int cols_) {
  rows = rows_;
  cols = cols_;
  cells.assign(static_cast<size_t>(rows) * cols, true);
}

void Boundary::clear() noexcept { std::fill(cells.begin(), cells.end(), true); }
...
bool Boundary::contains(float worldX, float worldY,
                        const GridRect &gridRect) const noexcept {
  int r, c;
  if (!cellAt(worldX, worldY, gridRect, r, c))
    return false;
...
void Boundary::render(SDL_Renderer *renderer, const GridRect &gridRect,
  ...
  for (int r = 0; r < rows; ++r) {
    for (int c = 0; c < cols; ++c) {
      // Draw only the blocked / unselected cells
      ...
      int sx = static_cast<int>(
          std::floor((world_x - centerX) * scale + centerX + 0.5f));
      int sy = static_cast<int>(
          std::floor((world_y - centerY) * scale + centerY + 0.5f));
